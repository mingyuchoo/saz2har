...
HarBuilder.decodeChunks = function (data) {
  ...
  while (process) {
    ...
    if (chunkSize) {
      const chunk = data.substr(
        index + delimeter.length + chunkSizeHex.length,
        chunkSize
      );
...
HarBuilder.prototype._parseQuery = function (query) {
  ...
  //skip fiddler error
  if (
    query.response.status.code === 502 ||
    query.response.status.code === 401
  ) {
    return;
  }
  ...
  const needToConvertToBase64 = !!/(image|font)/.test(
    getResponseHeaderValue('Content-Type')
  );
  const isCompressedBody = !!/(gzip|deflate)/.test(
    getResponseHeaderValue('Content-Encoding')
  );

  const isChunkedBody = !!/(chunked)/.test(
    getResponseHeaderValue('Transfer-Encoding')
  );
  ...
  if (responseContentType && responseContentType.indexOf('charset=') !== -1) {
    const responseCharsetMatches = responseContentType.match(
      'charset=([-_a-zA-Z0-9]+)'
    );
  ...
  const requestHeadersSize = (
    query.raw.request.split(this.END_OF_HTTP_HEADER)[0] +
    this.END_OF_HTTP_HEADER
  ).length;

  const responseHeadersSize = (
    query.raw.response.split(this.END_OF_HTTP_HEADER)[0] +
    this.END_OF_HTTP_HEADER
  ).length;
  ...
  that.FIDDLER_SESSION_TIMERS.forEach((name) => {
    timing[utils.uncapitalizeFirstLetter(name)] = moment(
      query.metadata.SessionTimers[name]
    );
  });

  entry.timings.send =
    timing.clientDoneRequest.diff(timing.clientBeginRequest) + 1;

  entry.timings.wait =
    timing.clientBeginResponse.diff(timing.clientDoneRequest) + 1;

  entry.timings.receive =
    timing.clientDoneResponse.diff(timing.clientBeginResponse) + 1;
