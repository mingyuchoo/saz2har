...
HttpParser.prototype.parseRequest = function (request) {
  ...
  const headers = this.request
    .substr(0, bodySeparatorIndex)
    .split(this.LINE_SEPARATOR);
  const body = this.request.substr(
    bodySeparatorIndex + this.BODY_SEPARATOR.length,
    this.request.length
  );
  ...
  headers.forEach((header, i) => {
    ...
    const headerValue = header
      .substr(
        headerSeparatorIndex + this.HEADER_SEPARATOR.length,
        header.length
      )
      .trim();
...
HttpParser.prototype.parseResponse = function (response) {
  ...
  const headers = this.response
    .substr(0, bodySeparatorIndex)
    .split(this.LINE_SEPARATOR);
  const body = this.response.substr(
    bodySeparatorIndex + this.BODY_SEPARATOR.length,
